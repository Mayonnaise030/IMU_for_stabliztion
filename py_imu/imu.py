    ...
    def getmeasure(self):
        ...
        while serial_port.in_waiting > 0 and self.newData == False:
            ...
            for i in range(self.len - 2):
                if self.receivedBytes[i] == b'\x20' and self.receivedBytes[i+1] == b'\x10' and self.receivedBytes[i+2] == b'\x10':
                    # print(i)
                    if i < 6:
                        self.q_num = i
                        self.len = self.q_num + 20

    def parseData(self):
       
        # -----QUAT = 0x2010-----
        if self.receivedBytes[self.q_num] == b'\x20' and self.receivedBytes[self.q_num+1] == b'\x10' and self.receivedBytes[self.q_num+2] == b'\x10':

            for i in range(16):
                self.data_q[i] = self.receivedBytes[self.q_num+3+i]      #進行資料的讀取
